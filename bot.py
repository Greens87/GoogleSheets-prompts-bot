...
bot_active = True
current_model = "gpt-4o"  # <-- Используем gpt-4o вместо gpt-4o-mini
...
def generate(update, context):
    ...
    /generate <count> <prompt>
      ...
      - 28..44 => warning
      - >= 45 => ок
      - Убираем самовольные --ar, --s, --style, --no logo
      - Тоже вырезаем “--Capture--”, “--Include--”, и т.д. (любое “--…” не являющееся командами)
      - Температура = 1.0
    """
    global bot_active
    if not bot_active:
    ...
    try:
        while count_generated < count and attempts < max_attempts:
            ...
            response = openai.ChatCompletion.create(
                ...
                temperature=1.0  # вы выбрали 1.0
            )
            ...
            # 2) Удаляем любые random двойные тире, кроме (ar|s|style|no)
            #   Пример: --Capture--, --Include-- и т.д.
            raw_text = re.sub(
                r'--(?!ar|s|style|no)(\S*)',
                '',
                raw_text,
            ...
            # 3) Удаляем официальные команды GPT мог написать (не те аргументы и т.д.)
            ...
            # 4) Удаляем крайние кавычки
            if raw_text.startswith('"') and raw_text.endswith('"'):
                raw_text = raw_text[1:-1].strip()

            # 5) Если пользователь не вводил кавычки
            ...
            # 7) Добавляем корректную строку параметров
            ...
            if word_count < 28:
                logger.warning(f"[Attempt {attempts}] ОТКЛОНЁН промт: {word_count} слов (<28).")
                time.sleep(1.0)
                continue
            elif word_count < 45:
